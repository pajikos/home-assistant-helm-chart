...
| `networkPolicy.cilium.enableDefaultDeny` | Cilium enableDefaultDeny parameter | `{}` |
| `networkPolicy.cilium.ingress` | Cilium network policy ingress https://docs.cilium.io/en/stable/security/policy/ | `[{"toPorts":[{"ports":[{"port":"http"}]}]}]` |
...
| `networkPolicy.ingress`  | Kubernetes ingress spec https://kubernetes.io/docs/concepts/services-networking/network-policies/| `[{"ports":[{"port":"http","protocol":"TCP"}]}]` |
| `networkPolicy.policyTypes`  | Kubernetes enforce policyTypes instead of default inferred one https://kubernetes.io/docs/concepts/services-networking/network-policies/ | `[]` |
...
You can define a network policy attached to the pod generated by this chart.
Two flavors are available, Kubernetes native policy or Cilium CNI policy, you can used one or other according to your setup and to the control level you want on the policy, because cilium allow more control on DNS queries for exemple.

Default values for network policy consist only in ingress flow allowed for http port.

Exemple of fully enforced Cilium with DNS filtering, allowing NodeRED ingress and MQTT egress
...
networkPolicy:
  ...
  cilium:
    ...
    # HTTPS
    - toFQDNs: &dns_names
      - matchName: aa015h6buqvih86i1.api.met.no
      - matchName: account-link.nabucasa.com
      - matchName: alerts.home-assistant.io
      - matchName: brands.home-assistant.io
      - matchName: data-v2.hacs.xyz
      - matchName: de1.api.radio-browser.info
      - matchName: files.pythonhosted.org
      - matchName: _api._tcp.radio-browser.info
      - matchName: mobile-apps.home-assistant.io
      - matchName: wheels.home-assistant.io
      toPorts:
      - ports:
        - port: "443"
          protocol: TCP
    ...
    - toEndpoints:
      - matchLabels:
          ...
          k8s:app.kubernetes.io/instance: mosquitto
          k8s:app.kubernetes.io/name: mosquitto
      toPorts:
      - ports:
        - port: '1883'
          protocol: TCP
    ...
    - fromEndpoints:
      - matchLabels:
          k8s:app.kubernetes.io/instance: nodered
          k8s:app.kubernetes.io/name: node-red
          k8s:io.kubernetes.pod.namespace: nodered
```

Exemple with simple Kubernetes with minimal filtering
...
networkPolicy:
  ...
  - from:
    - namespaceSelector:
        matchLabels:
          kubernetes.io/metadata.name: nodered
      podSelector:
        matchLabels:
